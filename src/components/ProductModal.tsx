    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-card" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="modal-layout">
          {!imageError ? (
            <img src={product.image} alt={product.title} className="modal-image" onError={() => setImageError(true)} />
          ) : (
            <div className="modal-image image-fallback" aria-hidden>{product.title?.charAt(0) ?? '🍽'}</div>
          )}
